...
class HomeController
{
    public function index()
    {
        $revenueChart  = null;
        $paymentsTable = null;
        $userScans = auth()->user()->qrScans;
        if($userScans){ $userScans=collect([]); }
        $totalValue = $userScans->sum('service_value');
        if (!auth()->user()->is_admin) {
            return view('home', compact('revenueChart', 'paymentsTable', 'userScans', 'totalValue'));
        }
